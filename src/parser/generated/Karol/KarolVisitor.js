// Generated from D:/projects/react-three-karol/antlrgrammar\Karol.g4 by ANTLR 4.9.2
// jshint ignore: start
import antlr4 from "antlr4";

// This class defines a complete generic visitor for a parse tree produced by KarolParser.

export default class KarolVisitor extends antlr4.tree.ParseTreeVisitor {
  // Visit a parse tree produced by KarolParser#karol.
  visitKarol(ctx) {
    return this.visitChildren(ctx);
  }

  // Visit a parse tree produced by KarolParser#definition.
  visitDefinition(ctx) {
    return this.visitChildren(ctx);
  }

  // Visit a parse tree produced by KarolParser#methoddefinition.
  visitMethoddefinition(ctx) {
    return this.visitChildren(ctx);
  }

  // Visit a parse tree produced by KarolParser#conditiondefinition.
  visitConditiondefinition(ctx) {
    return this.visitChildren(ctx);
  }

  // Visit a parse tree produced by KarolParser#statement.
  visitStatement(ctx) {
    return this.visitChildren(ctx);
  }

  // Visit a parse tree produced by KarolParser#iteration.
  visitIteration(ctx) {
    return this.visitChildren(ctx);
  }

  // Visit a parse tree produced by KarolParser#loop.
  visitLoop(ctx) {
    return this.visitChildren(ctx);
  }

  // Visit a parse tree produced by KarolParser#conditional.
  visitConditional(ctx) {
    return this.visitChildren(ctx);
  }

  // Visit a parse tree produced by KarolParser#instruction.
  visitInstruction(ctx) {
    return this.visitChildren(ctx);
  }

  // Visit a parse tree produced by KarolParser#parameterizedinstruction.
  visitParameterizedinstruction(ctx) {
    return this.visitChildren(ctx);
  }

  // Visit a parse tree produced by KarolParser#conditionexpression.
  visitConditionexpression(ctx) {
    return this.visitChildren(ctx);
  }

  // Visit a parse tree produced by KarolParser#condition.
  visitCondition(ctx) {
    return this.visitChildren(ctx);
  }

  // Visit a parse tree produced by KarolParser#parameterizedcondition.
  visitParameterizedcondition(ctx) {
    return this.visitChildren(ctx);
  }

  // Visit a parse tree produced by KarolParser#color.
  visitColor(ctx) {
    return this.visitChildren(ctx);
  }

  // Visit a parse tree produced by KarolParser#number.
  visitNumber(ctx) {
    return this.visitChildren(ctx);
  }
}
